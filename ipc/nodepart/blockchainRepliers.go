...
func (part *NodePart) replyToBlockchainGetDCDTToken(request common.MessageHandler) common.MessageHandler {
	typedRequest := request.(*common.MessageBlockchainGetDCDTTokenRequest)
	dcdtData, err := part.blockchain.GetDCDTToken(typedRequest.Address, typedRequest.TokenID, typedRequest.Nonce)
	response := common.NewMessageBlockchainGetDCDTTokenResponse(dcdtData, err)
	return response
}
